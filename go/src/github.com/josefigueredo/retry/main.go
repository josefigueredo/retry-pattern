...
func main() {
	...
	{ // drop in replacement for http.Get and other client methods
		resp, err := pester.Get("http://exampleee.com/foo/bar")
		if err != nil {
	...
	{ // control the resiliency
		...
		resp, err := client.Get("http://exampleee.com/foo/bar")
		if err != nil {
	...
	{ // use the pester version of http.Client.Do
		req, err := http.NewRequest("POST", "http://exampleee.com/foo/bar", strings.NewReader("data"))
		if err != nil {
